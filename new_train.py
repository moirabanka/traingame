...
def start():
    from alt_narration import help_text, intro
    print(help_text)
    global status, name
    status = 'normal'
...
def die(why):
    from alt_narration import game_over, error
    print(why)
...
def command_checker(checked_command):
    ...
    if checked_command not in valid_commands:
        from alt_narration import error
        print(error)
        return False
    else:
        return True
...
def target_checker(checked_target):
    from alt_narration import valid_targets
...
def simple_narrate(command, target):
    from alt_narration import normal, wizard, cursed
...
def car1():
    ...
    while location == 'car1':
        ...
        if command == 'touch' and ('tome' in target or 'thing' in target) and status == 'normal':
            ...
            from alt_narration import cursed
